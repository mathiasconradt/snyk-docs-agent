...
Add an annotation to the workload with the key orgs.k8s.snyk.io/v1, and add the Organization ID as the value in a comma-separated list.
...
After it is imported, the Project remains in your Snyk Organization even if you remove the annotation. To remove the Project from Snyk, you must delete the annotation and delete the Project from the Snyk UI or by using the API endpoint Delete a project.
