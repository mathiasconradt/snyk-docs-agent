To create a Snyk App, you must have access to the Snyk API. To get started, follow the instructions to authenticate for the API.
You must also retrieve the ID of the Snyk Organization you intend the App to be owned by (your orgId). You can get the Organization ID from the Organization settings in the Snyk Web UI or by using the List accessible organizations endpoint (https://api.snyk.io/rest/orgs ) with the Snyk API token in the Authorization header.
